function otp_login_index(path, url) {
  var tb = tables({
    width: [30, 25, 15, 25, 5],
    columns: [
    ...
    path: path,
    not_found_label: "Daftar Otp Login Tidak Ditemukan",
  });

  return tb;
}

function otp_login_start(path, url) {
  otp_login(100, path, url);
}

function otp_login(perpage, path, url, input) {
  var param = [];
  ...
  get_data(perpage, {
    ...
    fn: "List_" + path,
    warning_text:
      '<td colspan="4"><center>Daftar Otp Login Tidak Ditemukan</center></td>',
    param: param,
  });
}

function List_otp_login(JSONData) {
  ...
  var btn = [];
  btn[0] = btnDanger({
    title: "Delete Otp Login",
    onClick: ` onclick="delete_otp_login('${json.id}')" `,
    icon: "fas fa-times",
  });
...
function delete_otp_login(id) {
  ajax_default(
    {
      url: Urls("Otp_login/delete"),
      method: "post",
      loader: true,
      data: { id: id },
    },
    function (e, xhr) {
      ...
      otp_login(100, "otp_login", "Otp_login");
    },
