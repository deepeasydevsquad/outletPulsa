...
/**
 * Fungsi untuk melakukan Update produk Pascabayar IAK
 **/
controllers.update_produk_pascabayar_iak = async (req, res) => {
  const model_r = new Model_r(req);
  const process = await model_r.update_produk_pascabayar_iak();
  // error checking
  if (process.error) {
    res.status(400).json({
      error: true,
      error_msg: "Proses Update Produk Pascabayar IAK Berhasil Dilakukan",
    });
  } else {
    res.status(200).json({
      error: false,
      error_msg: "Proses Update Produk Pascabayar IAK Gagal Dilakukan",
    });
  }
};
