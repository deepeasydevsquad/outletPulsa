...
/**
 * Fungsi untuk melakukan delete produk prabayar
 *
 **/
controllers.delete_produk_prabayar = async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    // return ERROR
    const err_msg = await error_msg(errors);
    ...
    const model_cud = new Model_cud(req);
    // delete process
    await model_cud.delete_produk_prabayar();
    // get response
    if (await model_cud.response()) {
      res.status(200).json({
        error: false,
        error_msg: "Proses Menghapus Produk Baru Berhasil Dilakukan",
      });
    } else {
      res.status(400).json({
        error: true,
        error_msg: "Proses Menghapus Produk Baru Gagal Dilakukan.",
      });
    }
  }
};
